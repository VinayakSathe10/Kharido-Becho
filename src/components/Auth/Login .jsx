...
if (isBuyer) {
  try {
    ...
    if (!buyerData) {
      toast.error("Failed to fetch buyer info");
      return;
    }
    ...
    if (!buyerId) {
      toast.error("Buyer ID not found from server");
      return;
    }
    ...
    // Save auth state correctly
    dispatch(
      setAuthState({
        token,
        sellerId: null,
        buyerId, // correct
        buyerProfile: {   // ⬅ FIXED HERE
          roles,
    ...
    console.error("Buyer Fetch Error:", buyerErr);
    toast.error("Unable to process buyer login");
    return;
  }
}
