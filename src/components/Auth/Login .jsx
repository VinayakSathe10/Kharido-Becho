...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const baseUserId = response.userId; // 10005 (user table ID)
      ...
      if (isBuyer) {
        ...
        // Correct IDs from backend:
        const buyerId = buyerData.buyerId; // 1
        const buyerUserId = buyerData.user.id; // 10005

        if (!buyerUserId || !buyerId) {
          console.error("Missing buyer IDs:", buyerData);
          toast.error("Error: Invalid buyer account from backend");
          return;
        }

        // Store correctly
        localStorage.setItem("buyerId", buyerId); // for GET
        localStorage.setItem("buyerUserId", buyerUserId); // for POST
